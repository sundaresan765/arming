...
void crtpCommanderRpytDecodeSetpoint(setpoint_t *setpoint, CRTPPacket *pk)
{
  ...
  if (rawThrust > 0)
  {
    if(distanceDown < takeOffHeight)
    {
      boosThrvelocity = true;
    }
    else
    {
      boosThrvelocity = false;
    }
    if(distanceDown < 0.05f){
      ThrustPos = true;
    }
    else{
      ThrustPos = false;
  ...
  if (altHoldMode && isArmsuccess)
  {
    ...
    if (rawThrust != 0)
    {
      if (rawThrust > 0 )
      {
        ...
        if(distanceDown < 10.0f){ //to rapidly increase the target altitude during manual thrust
          targetAltitude = targetAltitude + 0.05f;
        }
    ...
    else if (!isTakeOff && !takeoff_completed) // condition for takeoff is not pressed and the drone is not armed
    {
      // disarmMotor();
      setpoint->mode.z = modeDisable;
       //printf("!isTakeOff && !takeoff_completed\n");
    ...
    {
      if(ThrustPos && !isTakeOff){
        setpoint->mode.z = modeDisable;
        //printf("modeAbs is activated becuz height is less than 5 cm\n");
      }else{
