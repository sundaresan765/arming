...
bool boostTakeoffvelocity = false; // using for takeoff
bool boosThrvelocity = false; // using for takeoff manual thrust

float takeOffHeight = 0.50f;
...
{
    printf("inside wifi_event_handler\n");
    ...
    else if (event_id == WIFI_EVENT_AP_STADISCONNECTED)
    {
        ...
        // Reset all control states when a client disconnects
        // xQueueReset(udpDataRx);
        // xQueueReset(udpDataTx);
    ...
    while (true)
    {
        ...
        else
        {
            ...
            { // takeoff message
                ...
                // isTakeOff = false;
                if (!less_voltage && !isTakeOff && altHoldMode &&  /*tof_suc &&*/ isArmsuccess)
                {
                    // altHoldMode = true;
                    targetAltitude = takeOffHeight; // 0.50f; //distanceDown;
                    printf("Takeoff mode is activated with TOF  target altitude is %f \n", targetAltitude);
                    isTakeOff = true;
                    boostTakeoffvelocity = true;
                    // land_completed = false;
                    // landMode = false;
                }
                else
        ...
        if (distanceDown > 0.10f /*takeOffHeight*/ && altHoldMode && (isTake_thrust || isTake_button))
        { //
            takeoff_completed = true;
            takeoff_battery = true;
            f = true;
            negative_thrust=false;
            ThrustPos=false; // var for stoping motor on ground
            boostTakeoffvelocity = false;
            boosThrvelocity = false;
        ...
        if (takeoff_completed && landMode && distanceDown <= 0.065f)
        {
            ...
            // if(tof_suc)
            // {
            //     printf("TOF sensor not initialized send ack to app\n");
            //     uint8_t packet[4] = {0xCD, 0xBB, 0xAA, 0x98}; // This packet will be sent to app if the sensor failsafe triggers
            //     for (size_t i = 0; i < sizeof(packet); i++)
            //     {
         ...
         if(!tof_suc)
            {
                printf("TOF sensor not initialized send ack to app\n");
                uint8_t packet[4] = {0xCD, 0xCC, 0xAC, 0x46}; // This packet will be sent to app if the sensor failsafe triggers
                for (size_t i = 0; i < sizeof(packet); i++)
                {
...
static void sendBatteryVoltageTask(void)
{
    ...
    while (1)
    {
        ...
        if (voltage <= 3.7)
        {
            printf("Battery voltage is low: %f\n", voltage);
