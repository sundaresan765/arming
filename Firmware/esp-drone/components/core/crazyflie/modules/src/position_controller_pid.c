...
float takeoffVelocity = 0.50f; //1.00f; // 1.0f;
...
static float rpLimit = 5;//10; //20;
static float rpLimitOverhead = 1.10f;
// Velocity maximums
static float xyVelMax = 0.01;//0.10; //0.50f; //1.0f;
static float zVelMax = 0.50f; //1.0f;
static float velMaxOverhead = 0.06;//0.60f; //1.10f;
...

{
  ...
  if (tof_suc)
  {
    ...
    if(rawThrust > 0.0f && velocityAdjustment < 0.0f)
    {
      velocityAdjustment = -(velocityAdjustment); // will make velocityAdjustment positive 
    }
    // if(currentAltitude < takeOffHeight && (boosThrvelocity || boostTakeoffvelocity)){
    //   velocityAdjustment = 0.50f; // will make velocityAdjustment  
    // }
  ...
  if(currentAltitude < takeOffHeight && (boosThrvelocity || boostTakeoffvelocity)){
    velocityAdjustment = takeoffVelocity; // will make velocityAdjustment  
  }

  //printf("velocity Z is : %.2f \n", velocityAdjustment);
 // printf("velocity X and Y is : %.2f , %0.2f \n", velocity.x, velocity.y);
